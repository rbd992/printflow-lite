...
const IconMail = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="2" y="4" width="14" height="10" rx="2" stroke="currentColor" strokeWidth="1.4"/><path d="M2 6l7 4 7-4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
);
const IconLock = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="3.5" y="8" width="11" height="8" rx="2" stroke="currentColor" strokeWidth="1.4"/><path d="M6 8V5.5C6 3.843 7.343 2.5 9 2.5C10.657 2.5 12 3.843 12 5.5V8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><circle cx="9" cy="12" r="1.2" fill="currentColor"/></svg>
);
const IconEye = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M1.5 9C1.5 9 4 3.5 9 3.5C14 3.5 16.5 9 16.5 9C16.5 9 14 14.5 9 14.5C4 14.5 1.5 9 1.5 9Z" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><circle cx="9" cy="9" r="2.5" stroke="currentColor" strokeWidth="1.4"/></svg>
);
const IconEyeOff = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M2 2L16 16" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M7.05 7.05C6.7 7.4 6.5 7.88 6.5 8.4C6.5 9.78 7.62 10.9 9 10.9C9.52 10.9 10 10.7 10.35 10.35" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M3.5 5.5C2.4 6.7 1.5 8.2 1.5 9C1.5 9 4 14.5 9 14.5C10.2 14.5 11.3 14.1 12.2 13.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M14.5 12.5C15.6 11.3 16.5 9.8 16.5 9C16.5 9 14 3.5 9 3.5C7.8 3.5 6.7 3.9 5.8 4.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>
);
const IconCheck = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3.5 8.5L6.5 11.5L12.5 4.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
);
...
const MeshBackground = () => {
  ...
  useEffect(() => {
    ...
    let animId, time = 0;
    const orbs = [
      { x: 0.2, y: 0.3, r: 350, color: [30, 58, 138] },
      { x: 0.7, y: 0.2, r: 300, color: [49, 46, 129] },
      { x: 0.5, y: 0.7, r: 380, color: [15, 23, 42] },
      { x: 0.8, y: 0.8, r: 250, color: [30, 27, 75] },
    ];
    const resize = () => {
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('pf_remember_email');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If authStore.error was set, login failed
      ...
      if (rememberMe) {
        localStorage.setItem('pf_remember_email', email.trim());
      } else {
  ...
  return (
    <div className="pf-login-root">
      ...
      <motion.div className="pf-login-container" variants={containerVariants} initial="hidden" animate="visible">
        <motion.div className="pf-glass-card" variants={itemVariants}>
          <motion.div className="pf-brand" variants={itemVariants}>
            <div className="pf-logo-wrap"><PrintFlowLogo size={52} /></div>
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div className="pf-error-banner"
          ...
          <form onSubmit={handleSubmit} className="pf-form" autoComplete="off">
            <motion.div className="pf-field" variants={itemVariants}>
              ...
              <div className="pf-input-wrap">
                ...
                <input ref={emailRef} id="pf-email" type="email" className="pf-input"
                  placeholder="Enter email address" value={email}
                  onChange={(e) => { setEmail(e.target.value); setLocalError(''); clearError(); }}
                  disabled={isLoading || success} autoComplete="email" spellCheck="false" />
              </div>
            </motion.div>
            ...
            <motion.div className="pf-field" variants={itemVariants}>
              ...
              <div className="pf-input-wrap">
                ...
                <input id="pf-pass" type={showPassword ? 'text' : 'password'}
                  className="pf-input pf-input-password" placeholder="Enter password"
                  value={password}
                  onChange={(e) => { setPassword(e.target.value); setLocalError(''); clearError(); }}
                  disabled={isLoading || success} autoComplete="current-password" />
                <button type="button" className="pf-toggle-vis"
            ...
            <motion.div className="pf-row-between" variants={itemVariants}>
              <label className="pf-checkbox-label">
                <input type="checkbox" checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)} className="pf-checkbox-native" />
            ...
            <motion.div variants={itemVariants}>
              <button type="submit"
                ...
                <AnimatePresence mode="wait">
                  {success ? (
                    <motion.span key="ok" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="pf-btn-content">
                      <IconCheck /> Authenticated
                    </motion.span>
                  ) : isLoading ? (
                    <motion.span key="ld" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pf-btn-content">
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="pf-spinner">
                    ...
                    <motion.span key="go" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pf-btn-content">
                      Sign In
                    </motion.span>
                  )}
                </AnimatePresence>
