...
router.post('/', authenticate, authorize('owner', 'manager'), (req, res) => {
  const db = getDb();
  const { name, brand = 'generic', model, serial, ip_address, access_code, camera_url, connection_type = 'network', has_ams = 0, ams_count = 0, notes } = req.body;
  if (!name || !model || !serial) return res.status(400).json({ error: 'name, model, serial required' });
  try {
    const result = db.prepare(`INSERT INTO printers (name, brand, model, serial, ip_address, access_code, camera_url, connection_type, has_ams, ams_count, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(name, brand, model, serial, ip_address || null, access_code || null, camera_url || null, connection_type, has_ams ? 1 : 0, ams_count, notes || null);
...
router.patch('/:id', authenticate, authorize('owner', 'manager'), (req, res) => {
  ...
  const allowed = ['name','brand','model','serial','ip_address','access_code','camera_url','connection_type','has_ams','ams_count','is_active','notes'];
  const updates = {};
